    ...
    std::pair<StorePath, Input> fetch(ref<Store> store, const Input & _input) override
    {
            ...
            if (doFetch) {
                ...
                try {
                    ...
                    fetchOpts.insert(fetchOpts.end(), {
                        "--", actualUrl,
                        fmt("%s:%s", shallow ? input.getRev()->gitRev() : fetchRef, fetchRef)
                    });
        ...
        if (submodules) {
            ...
            if (isLocal) {
                ...
                pathToGitFolder = repoDir + "/" + gitDir;
            }
            ...
            /* Ensure that we use the correct origin for fetching
               ...
               URLs. */
            if (isLocal) {
                writeFile(pathToGitFolder + "/config", readFile(repoDir + "/" + gitDir + "/config"));
                ...
                runProgram("git", true, {
                    "--git-dir", pathToGitFolder,
                    "--work-tree", tmpDir,
                    "config", "remote.origin.url", actualUrl
                });
            };
